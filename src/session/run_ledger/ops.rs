...
mod tests {
    ...
    fn transition_unknown_run_returns_none() {
        let dir = TempDir::new().unwrap();
        let workspace_dir = test_workspace(&dir);
        let result = transition_agent_run_status(
            workspace_dir,
            "ghost",
            AgentRunStatus::Pending,
            None,
    ...
    fn claim_is_atomic_first_wins_then_already_claimed() {
        ...
        let second =
            claim_agent_team_task(workspace_dir, "team-1", "task-a", "m2", "tok-2").unwrap();
    ...
    fn claim_blocked_until_dependency_done() {
        ...
        let blocked =
            claim_agent_team_task(workspace_dir, "team-1", "task-b", "m1", "tok").unwrap();
        assert_eq!(
            blocked,
