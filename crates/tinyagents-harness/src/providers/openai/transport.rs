...
impl OpenAiModel {
    ...
    pub(super) fn translate_request_with(
        ...
        let coalesced_messages: &[Message] = if prompt_guided_tools {
            prompt_messages = crate::tool::coalesce_prompt_tool_results(&request.messages);
            instructed_messages =
                crate::tool::with_prompt_tool_instructions(&prompt_messages, &request.tools);
            &instructed_messages
        } else {
            &request.messages
        };
        ...
        let base_messages: &[Message] = if native_tools {
            ...
            user_normalized_messages = crate::tool::ensure_resolvable_user_turn(coalesced_messages);
        ...
        let preparation = crate::tool::SchemaPreparation::openai();
        let mut tools: Vec<ToolWire> = if prompt_guided_tools {
            Vec::new()
        } else {
            crate::tool::prepare_tool_schemas(&request.tools, &preparation)
                .into_iter()
                .map(|schema| ToolWire {
    ...
    fn provider_error(
        ...
        let retryable = crate::retry::classify_provider_failure(status, code.as_deref(), &message)
            .is_retryable();
        ProviderError {
...
impl<State: Send + Sync> ChatModel<State> for OpenAiModel {
    ...
    /// [`credential_fingerprint`][crate::cache::credential_fingerprint]
    /// first, because this string is folded into keys that reach logs, events,
    /// and durable cache files.
    fn cache_identity(&self) -> Option<String> {
        let endpoint = crate::cache::model_cache_identity(
            &self.provider,
            &self.model,
            &self.base_url,
    ...
    async fn invoke(&self, _state: &State, request: ModelRequest) -> Result<ModelResponse> {
        ...
        // returned unchanged.
        if crate::tool::should_recover(
            self.native_tools_enabled(),
            !request.tools.is_empty(),
            response.message.tool_calls.len(),
        ) {
            return Ok(crate::tool::apply_prompt_tool_calls(response));
        }
        Ok(response)
    }
    ...
    async fn stream(&self, _state: &State, request: ModelRequest) -> Result<ModelStream> {
        ...
        let mut scrubber = crate::tool::ToolCallStreamScrubber::new();
...
///   [`should_recover`](crate::tool::should_recover) — a native response
...
pub(super) fn clean_stream_item(
    ...
    scrubber: &mut crate::tool::ToolCallStreamScrubber,
    native: bool,
) -> Vec<ModelStreamItem> {
    match item {
        ...
        ModelStreamItem::Completed(response) => {
            ...
            let recovered =
                if crate::tool::should_recover(native, true, response.message.tool_calls.len()) {
