...
/// ids (`crate::tool::prompt`) so a transcript that mixes both — a
...
fn prepare_response_schema(schema: &Value, strict: bool) -> Value {
    let preparation = if strict {
        crate::tool::SchemaPreparation::openai().with_strict()
    } else {
        crate::tool::SchemaPreparation::openai()
    };
    crate::tool::prepare_parameters(schema, &preparation)
}
