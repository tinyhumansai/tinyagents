...
fn tool_from_result_carries_the_structured_artifact() {
    let mut result = crate::tool::ToolResult::text("c1", "query", "3 rows");
...
fn artifact_does_not_leak_into_message_text() {
    let mut result = crate::tool::ToolResult::text("c1", "query", "ok");
...
fn artifact_serde_defaults_and_skips() {
    ...
    let mut result = crate::tool::ToolResult::text("c1", "t", "ok");
...
fn tool_schemas_are_counted() {
    let schema = crate::tool::ToolSchema::new(
        "search",
        "Search the corpus",
