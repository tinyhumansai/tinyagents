...
use crate::capability_resolver::{DEFAULT_NODE_KINDS, bind_capabilities_with_registry};
...
pub(super) fn full_registry() -> CapabilityRegistry<TestState> {
    ...
    reg.register_model(
        "default",
        std::sync::Arc::new(crate::test::provenance_diff_testkit::testkit::EchoModel),
    )
    .unwrap();
    reg.register_tool(std::sync::Arc::new(
        crate::test::provenance_diff_testkit::testkit::NoopTool,
    ))
    .unwrap();
...
fn registry_bind_rejects_unknown_node_kind() {
    ...
    let reg = full_registry();
    let err = compile_source(src, &reg).unwrap_err();
    assert!(matches!(err, tinyagents_harness::error::TinyAgentsError::Compile(_)));
