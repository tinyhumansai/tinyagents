...
use tinyagents_language::lexer::tokenize;
use tinyagents_language::parser::{parse, parse_str};
use tinyagents_language::types::{SpannedToken, Token};
...
fn parse_accepts_a_lone_eof_token_as_an_empty_program() {
    let eof = vec![SpannedToken {
        token: Token::Eof,
        span: tinyagents_language::Span::new(1, 1),
    }];
